...
describe("attachment metadata policy", () => {
  it("normalizes accepted content types and rejects unsupported or oversized files", () => {
    ...
    expect(() => validateAttachmentMetadata({ size: 5 }, "application/zip"))
      .toThrow("Attachments must be a PNG, JPEG, GIF, WebP, PDF, or plain-text file")
    expect(() => validateAttachmentMetadata({ size: MESSAGE_ATTACHMENT_POLICY.maxSizeBytes + 1 }, "image/png"))
      .toThrow("Attachments can be at most 25 MB")
  })
})
