...
export const generatedConvexModules = (apiDeclaration) =>
  [...apiDeclaration.matchAll(/import type \* as \S+ from "\.\.\/(.+)\.js";/g)]
    .map((match) => match[1])
    .sort()

export const staleConvexBindings = (convexDir) => {
  const expected = expectedConvexModules(convexDir)
  const generated = generatedConvexModules(
    readFileSync(resolve(convexDir, "_generated/api.d.ts"), "utf8"),
  )
  return {
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  if (stale.missing.length > 0 || stale.removed.length > 0) {
    console.error("Convex generated bindings are stale. Run `pnpm convex:codegen` with deployment access and commit the result.")
